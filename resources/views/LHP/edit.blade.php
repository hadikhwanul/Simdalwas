                    ...
                    <form class="row g-3" action="{{ route('lhp.update', $lhp->slug) }}" method="POST"
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="no_lhp" class="form-control @error('no_lhp') is-invalid @enderror"
                                name="no_lhp" value="{{ old('no_lhp', $lhp->no_lhp) }}" required>
                            @error('no_lhp')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" id="tanggal_lhp"
                                ...
                                value="{{ old('tanggal_lhp', $lhp->tanggal_lhp) }}" required>
                            @error('tanggal_lhp')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="judul" name="judul"
                                ...
                                value="{{ old('judul', $lhp->judul) }}" required>
                            @error('judul')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select id="departemen_id" class="form-select @error('departemen_id') is-invalid @enderror"
                                ...
                                @foreach ($departemen as $depart)
                                    <option value="{{ $depart->id }}"
                                        {{ old('departemen_id', $lhp->departemen_id) == $depart->id ? 'selected' : '' }}>
                                        {{ $depart->departemen }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="auditor_id" class="form-select @error('auditor_id') is-invalid @enderror"
                                ...
                                @foreach ($auditor as $audit)
                                    <option value="{{ $audit->id }}"
                                        {{ old('auditor_id', $lhp->auditor_id) == $audit->id ? 'selected' : '' }}>
                                        {{ $audit->auditor }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="bidang" class="form-select @error('bidang') is-invalid @enderror" name="bidang">
                                ...
                                <option value="Komprehensif"
                                    {{ old('bidang', $lhp->bidang) == 'Komprehensif' ? 'selected' : '' }}>Komprehensif
                                </option>
                                <option value="Kebijakan"
                                    {{ old('bidang', $lhp->bidang) == 'Kebijakan' ? 'selected' : '' }}>Kebijakan</option>
                                <option value="Tupoksi" {{ old('bidang', $lhp->bidang) == 'Tupoksi' ? 'selected' : '' }}>
                                    Tupoksi</option>
                                <option value="Pengelolaan Aset Daerah"
                                    {{ old('bidang', $lhp->bidang) == 'Pengelolaan Aset Daerah' ? 'selected' : '' }}>
                                    Pengelolaan Aset Daerah</option>
                                <option value="Pengelolaan Keuangan"
                                    {{ old('bidang', $lhp->bidang) == 'Pengelolaan Keuangan' ? 'selected' : '' }}>
                                    Pengelolaan Keuangan</option>
                                <option value="Pengelolaan Pendapatan"
                                    {{ old('bidang', $lhp->bidang) == 'Pengelolaan Pendapatan' ? 'selected' : '' }}>
                                    Pengelolaan Pendapatan</option>
                                <option value="Pengelolaan Kepegawaian"
                                    {{ old('bidang', $lhp->bidang) == 'Pengelolaan Kepegawaian' ? 'selected' : '' }}>
                                    Pengelolaan Kepegawaian</option>
                                <option value="Pengelolaan Kekayaan"
                                    {{ old('bidang', $lhp->bidang) == 'Pengelolaan Kekayaan' ? 'selected' : '' }}>
                            ...
                            <select id="induk_id" class="form-select @error('induk_id') is-invalid @enderror"
                                ...
                                @foreach ($induk as $indukItem)
                                    <option value="{{ $indukItem->id }}"
                                        {{ old('induk_id', $lhp->induk_id) == $indukItem->id ? 'selected' : '' }}>
                                        {{ $indukItem->induk }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="sifat" class="form-select @error('sifat') is-invalid @enderror" name="sifat"
                                ...
                                <option value="Reguler" {{ old('sifat', $lhp->sifat) == 'Reguler' ? 'selected' : '' }}>
                                    Reguler</option>
                                <option value="Khusus" {{ old('sifat', $lhp->sifat) == 'Khusus' ? 'selected' : '' }}>
                                    Khusus</option>
                                <option value="Kinerja" {{ old('sifat', $lhp->sifat) == 'Kinerja' ? 'selected' : '' }}>
                                    Kinerja</option>
                                <option value="Rahasia" {{ old('sifat', $lhp->sifat) == 'Rahasia' ? 'selected' : '' }}>
                                    Rahasia</option>
                                <option value="Terpadu" {{ old('sifat', $lhp->sifat) == 'Terpadu' ? 'selected' : '' }}>
                                    Terpadu</option>
                                <option value="Kasus" {{ old('sifat', $lhp->sifat) == 'Kasus' ? 'selected' : '' }}>Kasus
                                </option>
                            </select>
                            @error('sifat')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            @if ($lhp->laporan)
                                <p class="text-muted mt-1">
                                    Current file:
                                    <a href="{{ asset($lhp->laporan) }}" target="_blank"
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="y" type="hidden" name="pemeriksa"
                                ...
                                value="{{ old('pemeriksa', $lhp->pemeriksa) }}">
